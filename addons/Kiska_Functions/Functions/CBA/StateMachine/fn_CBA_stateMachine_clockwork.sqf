/* ----------------------------------------------------------------------------
Function: KISKA_fnc_CBA_stateMachine_clockwork
...
---------------------------------------------------------------------------- */
scriptName "KISKA_fnc_CBA_stateMachine_clockwork";

KISKA_CBA_stateMachines apply {
    ...
    private _list = _stateMachine get "KISKA_CBA_stateMachine_list";
    private _skipNull = _stateMachine get "KISKA_CBA_stateMachine_skipNull";
    private _tick = _stateMachine get "KISKA_CBA_stateMachine_tick";
    if (_skipNull) then {
        while {(_tick < (count _list)) AND {isNull (_list select _tick)}} do {
            _tick = _tick + 1;
        };
    };
    ...
    private _id = _stateMachine get "KISKA_CBA_stateMachine_id";
